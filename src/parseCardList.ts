	...
	for (const card of customCards) {
		...
		if (card.draft_effects) {
			for (const effect of card.draft_effects) {
				if (effect.type === ParameterizedDraftEffectType.AddCards) {
				...
				} else if (effect.type === ParameterizedDraftEffectType.ReplaceNotedCard) {
					for (let i = 0; i < effect.cards.length; ++i) {
						const cid = effect.cards[i];
						// This is a valid card ID, nothing more to do.
						if (isValidCardID(cid)) continue;
					...
					if (effect.count <= 0 || effect.count > effect.cards.length) {
						return ackError({
							title: `Invalid Parameter`,
							text: `Invalid 'ReplaceNotedCards' entry in 'draft_effects' of '${card.name}'. 'count' (${effect.count}) must be strictly positive and less than or equal to the number of cards in 'cards' (${effect.cards.length}).`,
						});
					}
				} else if (effect.type === ParameterizedDraftEffectType.AddCardsOnReveal) {
					for (let i = 0; i < effect.cards.length; ++i) {
						const cid = effect.cards[i];
						// This is a valid card ID, nothing more to do.
						if (isValidCardID(cid)) continue;
					...
					if (effect.count <= 0 || effect.count > effect.cards.length) {
						return ackError({
							title: `Invalid Parameter`,
							text: `Invalid 'AddCardsOnReveal' entry in 'draft_effects' of '${card.name}'. 'count' (${effect.count}) must be strictly positive and less than or equal to the number of cards in 'cards' (${effect.cards.length}).`,
						});
					}
				}
			}
